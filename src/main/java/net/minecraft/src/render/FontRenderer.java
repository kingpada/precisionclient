...
public class FontRenderer {
    ...
    private static String getFormatFromString(String str) {
        ...
        while ((index = str.indexOf(167, index + 1)) != -1) {
            if (index < length - 1) {
                char foundChar = str.charAt(index + 1);

                if (isFormatColor(foundChar)) {
                    FormattedString = "\u00a7" + foundChar;
                } else if (isFormatSpecial(foundChar)) {
                    FormattedString = FormattedString + "\u00a7" + foundChar;
                }
            }
        }
